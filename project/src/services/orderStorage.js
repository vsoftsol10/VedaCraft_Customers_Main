...
export async function updateOrderStatus(orderId, status, userId) {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/orders/${encodeURIComponent(orderId)}/status`, {
      ...
      body: JSON.stringify({ status, updatedAt: nowIso }),
    });
    const result = await response.json().catch(() => ({}));

    if (!response.ok || !result.success) {
      console.error('Failed to update order status:', result.message || response.statusText);
      return null;
    }

    const resolvedUserId = await resolveUserId();
    const savedOrder = normalizeOrder(result.data, resolvedUserId);
