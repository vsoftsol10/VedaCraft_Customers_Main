...
// Days elapsed since a given date, used to gate the return window
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-8">
        ...
        <div className="flex flex-col sm:items-end gap-2">
          ...
          {/* Return button — Delivered orders only, within the return window */}
          {(withinReturnWindow || returnWindowExpired) && (
            <button
              type="button"
              onClick={handleReturnOrder}
              disabled={!withinReturnWindow || isReturning}
              title={
                !withinReturnWindow
              ...
              className={`inline-flex items-center justify-center gap-2 rounded-md border px-4 py-2 text-sm font-semibold transition-colors disabled:cursor-not-allowed disabled:opacity-60 ${
                withinReturnWindow
                  ? 'border-[#2d6a2d]/30 text-[#2d6a2d] hover:bg-[#f0f5ec]'
                  : 'border-gray-200 text-gray-400 bg-gray-50'
              }`}
            >
              <RotateCcw className="w-4 h-4" />
              {isReturning ? t('orders.returning', 'Processing...') : t('orders.returnOrder', 'Return Order')}
            </button>
          )}
        </div>
      </div>
      ...
      {order.status === 'Cancelled' ? (
        <div className="relative flex justify-between items-center mb-12 px-[10%]">
          ...
          <div className="relative flex flex-col items-center gap-2 text-center z-10 w-24">
            ...
            <div className="flex flex-col items-center mt-1">
              <span className="text-xs sm:text-sm font-semibold text-gray-900">{t('orders.ordered')}</span>
              <span className="text-[11px] text-gray-500 whitespace-nowrap mt-0.5">
        ...
        <div className="relative grid grid-cols-5 gap-2 mb-8 mt-4">
          ...
          {trackingSteps.map((step, index) => {
            ...
            return (
              <div key={step.labelKey} className="relative flex flex-col items-center gap-2 text-center">
                <div
      ...
      >
        {order.status === 'Cancelled' ? (
          ...
          <div className="flex items-start gap-3">
            <Home className="w-7 h-7 text-green-500 flex-shrink-0 mt-0.5" />
            <div>
              <p className="text-sm font-bold text-[#2d8f3a]">
                {t('orders.deliveredOn', {
                  date: formatDisplayDate(order.deliveredAt || order.createdAt),
                })}
              </p>
              <p className="text-xs text-gray-600 mt-1">{t('orders.deliveredDesc')}</p>
              {withinReturnWindow && (
                <p className="text-xs text-[#2d6a2d] font-medium mt-2">
                  {t(
                    'orders.returnWindowNote',
                    `Eligible for return within ${RETURN_WINDOW_DAYS} days of delivery.`
                  )}
                </p>
              )}
            </div>
      ...
      <div className="grid md:grid-cols-2 gap-5 mb-7">
        <section className="border border-gray-200 rounded-lg p-4">
          ...
          {order.address ? (
            ...
            <p className="text-sm text-gray-500">{t('orders.addressUnavailable')}</p>
          )}
        </section>
      ...
      <section>
        ...
        <div className="border border-gray-200 rounded-lg overflow-hidden">
          {order.items.length === 0 ? (
            <p className="p-4 text-sm text-gray-500">{t('orders.itemDetailsUnavailable')}</p>
          ) : (
